...
public class LoadBalancingPolicyBootstrapTest extends CCMTestsSupport {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(LoadBalancingPolicyBootstrapTest.class);
  ...
  /**
   * Ensures that {@link LoadBalancingPolicy#onDown(Host)} is called for a contact point that
   * couldn't be reached while initializing the control connection, but only after {@link
   * LoadBalancingPolicy#init(Cluster, Collection)} has been called.
   *
   * @test_category load_balancing:notification
   ...
   */
  @Test(groups = "long")
  public void should_send_down_notifications_after_init_when_contact_points_are_down()
      throws Exception {

    // In order to validate this behavior, we need to stop the first node that would be attempted to
    // be
    // established as the control connection.
    // This depends on the internal behavior and will even be made totally random by JAVA-618,
    // therefore
    ...
    for (; tries <= maxTries; tries++) {
      ...
      try {
        ...
        if (policy.history.contains(entry(DOWN, TestUtils.findHost(cluster, nodeToStop)))) {
          // This is the situation we're testing, the control connection tried the stopped node
          // first.
          assertThat(policy.history)
              .containsExactly(
